        ...
        <div class="players col-sm-3">
          <div class="card">
            ...
            <ul class="list-group list-group-flush">
              <li
                v-if="playersByScore"
                v-for="(player, i) in playersByScore"
                :key="i"
          ...
          <div class="card">
            <div
              ...
              <div class="float-left">
                {{ $t('Questions:') }}
                {{ `${serverState.questionsAsked}/${serverState.maxQuestions}` }}
              </div>
            </div>
            <div class="card-body">
              <div v-if="gameState === 'lobby'">
                <p class="card-text">
                  ...
                  {{ $t('Or go to:') }} <a
                    :href="gameURL"
                    target="_blank">{{ gameURL }}</a><br>
                </p>
              </div>

              <div
                class="state-question"
                v-else-if="gameState === 'question'">
                {{ serverState.question.text }}
              </div>
              <div
                class="state-vote"
                v-else-if="gameState === 'vote'">
                {{ serverState.question.text }}<br>
                {{ $t('Choose one of these answer:') }}<br>
                <div
                  class="badge badge-primary"
                  v-for="(answer, i) in serverState.answersChoice"
                  :key="i"
                >{{ answer }}</div>
              </div>
              ...
              <div v-else-if="gameState === 'results'">
                ...
                >
                  <div class="result">
                    <div
                      v-for="(result, i) in serverState.results"
                      :key="i"
                      class="vote"
                      :class="{
                      ...
                      <div class="label badge badge-primary">
                        ...
                        <span
                          ...
                          <template v-if="result.type === 'fool'">
                            <span
                              v-for="(player, i) in result.foolers"
                              :key="i"
            ...
            <div
              ...
              <span
                class="float-left"
                style="height: 25px; margin: 0 10px; font-weight: bold;">
...
export default {
  ...
  watch: {
    ...
    'serverState.gameTimer': function () {
  ...
  methods: {
    ...
    enter(el, done) {
      ...
      votes.reduce((promise, vote) => promise
        .then(() => {
...
<i18n>
{
  "fr": {
    ...
    "A player has been lost! The game will resume once they're back.":
      "Un joueur a été perdu! La partie reprendra une fois tout le monde revenue.",
    "Click start on your device to start the game.":
      "Cliquez \"Démarrer la partie\" sur votre appareil pour commencer la partie.",
